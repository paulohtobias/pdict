...
const char *pdict_get_last_error_message() {
	return pdict_error_messages[__pdict_errno];
}

const char *pdict_get_error_message(int pdict_errno) {
	return pdict_error_messages[pdict_errno];
}
...
static inline int32_t hash_abs(int32_t hash) {
	return hash > 0 ? hash : -hash;
}

static int32_t pdict_get_index(const pdict_t *dict, const char *key, bool find_empty) {
	...
	for (i = 0; i < 4; i++) {
		index = hash_abs(hash[i] % dict->max_len);
...
pdict_item_t *pdict_get_item(const pdict_t *dict, const char *key) {
	...
	int32_t index = pdict_get_index(dict, key, false);
...
int pdict_set_value_all(pdict_t *dict, const char *key, void *value, bool add_missing_keys, void (*free_value)(void *)) {
	...
	if (item == NULL) {
		if (add_missing_keys) {
			int32_t index = pdict_get_index(dict, key, true);
			if (index < 0) {
				return 1;
			}
...
char **pdict_get_keys(const pdict_t *dict, int32_t *keys_len, bool sort) {
	char **keys = calloc(dict->len, sizeof(char *));
	int32_t i, j, k;
